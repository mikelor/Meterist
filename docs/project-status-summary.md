...
## 3. Ten insights we can report on right now
...
| 10 | **Cost projection** — forecast next period's spend per tenant/vendor, using the same three scenario methodologies the original spreadsheet used (flat, trend-adjusted, latest-period-annualized), run against real daily-grain data instead of manual weekly guesses | This was the spreadsheet's *original* purpose — its whole "Annual Projection" tab existed to answer "what will this cost us going forward," and it's confirmed v1 scope (`product-design-document.md` §6, item 7), not backlog. Everything built so far (multi-tenant, versioned rates, daily aggregation across all four vendors) has been the data foundation this needs; it's arguably the last confirmed v1 feature not yet implemented | P1 | Medium |
...
**On #10 specifically:** don't confuse this with #1's "run-rate projection," which is a simple linear extrapolation of the current tracked period (useful for a one-line KPI, not a real forecast). #10 is the fuller model the spreadsheet actually used — three distinct scenarios per vendor (flat continuation, trend-adjusted, latest-period-annualized), each a different assumption about how usage evolves, not just one extrapolated number. That's meaningfully more work (per-vendor trend detection, scenario logic, probably a way to present three numbers without overwhelming the one-slide takeaway #1 is going for), which is why it's tracked as its own item rather than folded into #1. It's also the first insight on this list that depends on having *enough* historical data to trend against, rather than just a snapshot — worth revisiting once a few more weeks of live-extracted data accumulate for both tenants.
...
5. Once the #1+#2+#3 bundle and #9 are live, take on #10 (cost projection) — it directly completes the spreadsheet-replacement mandate this whole project started from, and needs a few more weeks of accumulated daily data to trend against before it's meaningful.
